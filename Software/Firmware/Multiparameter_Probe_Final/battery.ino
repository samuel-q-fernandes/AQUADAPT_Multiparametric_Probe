...
#define IP5306_SetChargingFullStopVoltage(v)    ip5306_set_bits(IP5306_REG_CHG_0, 0, 1, v)//*0:4.14V, 1:4.17V, 2:4.185V, 3:4.2V (values are for charge cutoff voltage 4.2V, 0 or 1 is recommended)
...
#define IP5306_SetEndChargeCurrentDetection(v)  ip5306_set_bits(IP5306_REG_CHG_1, 6, 1, v)//0:200mA, *1:400mA, 2:500mA, 3:600mA
...
#define IP5306_SetChargeCutoffVoltage(v)        ip5306_set_bits(IP5306_REG_CHG_2, 2, 0, v)//*0:4.2V, 1:4.3V, 2:4.35V, 3:4.4V
...
float printIP5306Stats(){
    ...
    //Serial.println(IP5306_LEDS2PCT(leds));
    //float levelBat = IP5306_LEDS2PCT(leds);
    //return levelBat;
}
